...
public class NoWhitespaceAfterKeyword :
  ...
  private fun checkKeywordSpacing(
    ...
    val nextSibling = keyword.nextSibling { !it.isWhiteSpace20 }
    val whitespace = keyword.treeNext

    if(whitespace != null && whitespace.isWhiteSpace20 && whitespace != nextSibling) {
      emit(
        keyword.startOffset,
  ...
  private fun removeWhitespaceBetween(keyword: ASTNode, nextNonWhitespace: ASTNode?) {
    ...
    while(current != null && current.isWhiteSpace20 && current != nextNonWhitespace) {
